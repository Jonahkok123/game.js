/* =====================================================
   INTO DARKNESS — BUGFIX & INTERACTION FIX PATCH
   ===================================================== */

const canvas = document.getElementById("c");
const ctx = canvas.getContext("2d");

// ------------------ SETTINGS ------------------
const PLAYER_RADIUS = 12;
const ENEMY_RADIUS = 12;

// ------------------ INPUT ------------------
let keys = {}, mouse = { x:0,y:0,down:false,click:false };
addEventListener("keydown",e=>keys[e.key.toLowerCase()]=true);
addEventListener("keyup",e=>keys[e.key.toLowerCase()]=false);
canvas.addEventListener("mousemove",e=>{
  const r=canvas.getBoundingClientRect();
  mouse.x=e.clientX-r.left;mouse.y=e.clientY-r.top;
});
canvas.addEventListener("mousedown",()=>{mouse.down=true;mouse.click=true});
canvas.addEventListener("mouseup",()=>mouse.down=false);

// ------------------ PLAYER ------------------
const player={
  x:300,y:300,
  hp:150,maxHp:150,
  dmg:6,speed:3,
  gold:0,cooldown:0,
  equip:{weapon:null,chest:null}
};

// ------------------ WORLD ------------------
let enemies=[],bullets=[],loot=[],boss=null;
let world={x:0,y:0};

// ------------------ ITEMS ------------------
function equipItem(item){
  if(item.slot){
    player.equip[item.slot]=item;
  }
}
const WEAPONS=[
  {name:"Iron Sword",slot:"weapon",dmg:3},
  {name:"Phoenix Staff",slot:"weapon",dmg:6,fire:true}
];
const ARMOUR=[
  {name:"Chainmail",slot:"chest",armour:0.25}
];

// ------------------ NPCs ------------------
const blacksmith={x:200,y:200,type:"blacksmith",open:false};
const enchanter={x:400,y:200,type:"enchanter",open:false};

// ------------------ SPAWN ------------------
function spawnRoom(){
  enemies=[];bullets=[];loot=[];boss=null;
  player.x=canvas.width/2;player.y=canvas.height/2;

  if(world.y===-1){
    boss={x:canvas.width/2,y:120,hp:300,phase:1,cd:60};
    return;
  }

  if(world.x===0&&world.y===0)return;

  for(let i=0;i<4;i++){
    enemies.push({x:100+Math.random()*400,y:100+Math.random()*300,hp:40,atk:8});
  }
}
spawnRoom();

// ------------------ UPDATE ------------------
function update(dt){
  // Player movement
  let mx=(keys.d?1:0)-(keys.a?1:0);
  let my=(keys.s?1:0)-(keys.w?1:0);
  let m=Math.hypot(mx,my)||1;
  player.x+=mx/m*player.speed*dt;
  player.y+=my/m*player.speed*dt;

  // Shooting
  if(mouse.down&&player.cooldown<=0){
    let dx=mouse.x-player.x,dy=mouse.y-player.y,d=Math.hypot(dx,dy)||1;
    bullets.push({x:player.x,y:player.y,dx:dx/d*8,dy:dy/d*8,dmg:player.dmg+(player.equip.weapon?.dmg||0)});
    player.cooldown=15;
  }
  if(player.cooldown>0)player.cooldown-=dt;

  bullets.forEach(b=>{b.x+=b.dx*dt;b.y+=b.dy*dt});

  // -------- COLLISION FIX --------
  enemies.forEach(e=>{
    let dx=player.x-e.x,dy=player.y-e.y;
    let dist=Math.hypot(dx,dy)||1;
    if(dist<PLAYER_RADIUS+ENEMY_RADIUS){
      let overlap=(PLAYER_RADIUS+ENEMY_RADIUS-dist)/2;
      player.x+=dx/dist*overlap;
      player.y+=dy/dist*overlap;
      e.x-=dx/dist*overlap;
      e.y-=dy/dist*overlap;

      player.hp-=e.atk*dt; // Enemy can hurt player ✅
    }
  });

  // Enemy movement
  enemies.forEach(e=>{
    let dx=player.x-e.x,dy=player.y-e.y,d=Math.hypot(dx,dy)||1;
    e.x+=dx/d*1.2*dt;
    e.y+=dy/d*1.2*dt;
  });

  // Bullet damage
  bullets.forEach(b=>{
    enemies.forEach(e=>{
      if(Math.hypot(b.x-e.x,b.y-e.y)<ENEMY_RADIUS){
        e.hp-=b.dmg;b.dead=true;
      }
    });
    if(boss&&Math.hypot(b.x-boss.x,b.y-boss.y)<30){
      boss.hp-=b.dmg;b.dead=true;
    }
  });
  bullets=bullets.filter(b=>!b.dead);

  // -------- BOSS PHASE FIX --------
  if(boss){
    if(boss.hp<200)boss.phase=2;
    if(boss.hp<100)boss.phase=3;
    boss.cd--;
    if(boss.cd<=0){
      boss.cd=50-boss.phase*10;
      for(let i=0;i<boss.phase*6;i++){
        let a=i*(Math.PI*2/(boss.phase*6));
        bullets.push({x:boss.x,y:boss.y,dx:Math.cos(a)*3,dy:Math.sin(a)*3,dmg:5});
      }
    }
  }

  // Death & loot variety fix ✅
  enemies=enemies.filter(e=>{
    if(e.hp<=0){
      loot.push({x:e.x,y:e.y,gold:2+Math.floor(Math.random()*6)});
      if(Math.random()<0.3)loot.push({x:e.x+8,y:e.y,item:WEAPONS[Math.floor(Math.random()*WEAPONS.length)]});
      return false;
    }
    return true;
  });

  // Pickup
  loot=loot.filter(l=>{
    if(Math.hypot(player.x-l.x,player.y-l.y)<20){
      if(l.gold)player.gold+=l.gold;
      if(l.item)equipItem(l.item);
      return false;
    }
    return true;
  });

  mouse.click=false;
}

// ------------------ DRAW ------------------
function draw(){
  ctx.fillStyle="#111";
  ctx.fillRect(0,0,canvas.width,canvas.height);

  // NPCs ✅
  ctx.fillStyle="orange";
  ctx.fillRect(blacksmith.x-10,blacksmith.y-20,20,40);
  ctx.fillStyle="purple";
  ctx.fillRect(enchanter.x-10,enchanter.y-20,20,40);

  // Player
  ctx.fillStyle="white";
  ctx.beginPath();
  ctx.arc(player.x,player.y,PLAYER_RADIUS,0,Math.PI*2);
  ctx.fill();

  // Enemies
  ctx.fillStyle="red";
  enemies.forEach(e=>{
    ctx.beginPath();
    ctx.arc(e.x,e.y,ENEMY_RADIUS,0,Math.PI*2);
    ctx.fill();
  });

  // Boss
  if(boss){
    ctx.fillStyle=["orange","crimson","gold"][boss.phase-1];
    ctx.fillRect(boss.x-24,boss.y-24,48,48);
  }

  // Bullets
  ctx.fillStyle="yellow";
  bullets.forEach(b=>ctx.fillRect(b.x-2,b.y-2,4,4));

  // HUD
  ctx.fillStyle="red";
  ctx.fillRect(20,20,(player.hp/player.maxHp)*200,8);
  ctx.fillStyle="white";
  ctx.fillText("Gold: "+player.gold,20,50);
  ctx.fillText("Weapon: "+(player.equip.weapon?.name||"None"),20,70);
}

// ------------------ LOOP ------------------
let last=0;
function loop(t){
  let dt=Math.min((t-last)/16.6,2);
  last=t;
  update(dt);
  draw();
  requestAnimationFrame(loop);
}
requestAnimationFrame(loop);
